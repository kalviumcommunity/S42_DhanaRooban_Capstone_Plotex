...
We convert the latitude and longitude to radians using parseFloat().
We create a Point geometry object with the coordinates.
We query the RentalUser collection using $nearSphere operator to find documents within the specified distance.
The $maxDistance parameter is set to maxDistance * 1609.34, which converts meters to miles (1 mile = 1609.34 meters).
We return the found nearby locations.
